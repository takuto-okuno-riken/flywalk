...
%}
    % ---------------------------------------------------------------------
    % make structural connectivity matrix of hemibrain primary ROI atlas (FlyWire compatible format)
%%{
    rateThs = [80];
    synThs = 0;
%    rateThs = [80];
%    synThs = 5;
    for r=1:length(rateThs)
        ...
        for j=1:length(synThs)
            synTh = synThs(j);
            idstr = ['hemiroi_hb' num2str(synTh) 'sr' num2str(rateTh) 'fw'];
            fname = ['results/sc/' lower(idstr) '_connectlist.mat'];
            ...
            if exist(fname,'file')
                ...
                conf = getSCconfig('hemi', synTh, rateTh);
